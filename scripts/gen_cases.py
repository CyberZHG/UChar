...
with open("include/unicode_data.h", "a") as writer:
    ...
    writer.write("constexpr int UPPER_NUM = {};\n".format(len(uppers)))
    writer.write(
    ...
    writer.write("constexpr int LOWER_NUM = {};\n".format(len(lowers)))
    writer.write(
        "/** The indices of the first character that have a different type. */\n"
    )
    ...
    writer.write("constexpr int TITLE_NUM = {};\n".format(len(titles)))
    writer.write(
        "/** The indices of the first character that have a different type. */\n"
    )
...
with open("src/cases.cpp", "w") as writer:
    ...
    for name, cases in zip(["UPPER", "LOWER", "TITLE"], [uppers, lowers, titles]):
