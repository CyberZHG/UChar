...
with open("src/decomposition_mapping.cpp", "w") as writer:
    ...
    for tag in unique_tags:
        writer.write(
            "constexpr auto {} = DecompositionMappingTag::{};\n".format(tag, tag)
        )
    writer.write("\n")
...
"""
    ...
    for code, tag, decomposition in zip(codes, tags, decompositions):
        if tag not in appeared:
            appeared.add(tag)
            writer.write(
                "    EXPECT_EQ(unicode::DecompositionMappingTag::{}, "
                "unicode::getDecompositionMappingTag({})); {{\n".format(
                    tag, "0x" + code
                )
            )
            writer.write(
                "        auto decomposition = unicode::getDecompositionMapping({});\n".format(
                    "0x" + code
                )
            )
            writer.write(
                "        EXPECT_EQ({}u, decomposition.size());\n".format(
                    len(decomposition)
                )
            )
            for i in range(len(decomposition)):
                writer.write(
                    "        EXPECT_EQ({}, decomposition[{}]);\n".format(
                        "0x" + decomposition[i], i
                    )
                )
            writer.write(
                "        unicode::getDecompositionMapping({}, buffer);\n".format(
                    "0x" + code
                )
            )
            for i in range(len(decomposition)):
