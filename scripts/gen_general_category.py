...
with open("src/general_category.cpp", "w") as writer:
    ...
    for category in sorted(list(set(base_categories))):
        writer.write(
            '    case BaseGeneralCategory::{}: os << "{}"; break;\n'.format(
                category, category
            )
        )
    writer.write("    }\n")
    ...
    with open("UnicodeData.txt", "r") as reader:
        for line in reader:
            ...
            if category not in appeared:
                ...
                writer.write(
                    "    EXPECT_EQ(unicode::GeneralCategory::{}, unicode::getGeneralCategory({}));\n".format(
                        category, "0x" + parts[0]
                    )
                )
                writer.write(
                    f"""    ss << unicode::GeneralCategory::{category};
    ...
    with open("UnicodeData.txt", "r") as reader:
        for line in reader:
            ...
            if category not in appeared:
                appeared.add(category)
                writer.write(
                    "    EXPECT_EQ(unicode::BaseGeneralCategory::{}, "
                    "unicode::getBaseGeneralCategory({}));\n".format(
                        category, "0x" + parts[0]
                    )
                )
                writer.write(
                    f"""    ss << unicode::BaseGeneralCategory::{category};
    EXPECT_EQ("{category}", ss.str());
    ss.str("");
    ss.clear();
"""
